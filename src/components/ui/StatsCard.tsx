...
const colorMap = {
  ...
  success: { bg: "bg-success/10", text: "text-success", border: "border-success/20", gradient: ["#34D399", "#2BBF89"] },
  warning: { bg: "bg-warning/10", text: "text-warning", border: "border-warning/20", gradient: ["#FBBF24", "#F59E0B"] },
  error: { bg: "bg-error/10", text: "text-error", border: "border-error/20", gradient: ["#F87171", "#EF4444"] },
  info: { bg: "bg-info/10", text: "text-info", border: "border-info/20", gradient: ["#818CF8", "#6366F1"] },
}
...
export function StatsCard({ title, value, icon: Icon, trend, sparkline, color = "primary", description, className, formatValue }: StatsCardProps) {
  ...
  return (
    ...
    >
      ...
      <p className={cn("text-[28px] font-bold leading-tight", palette.text)}>
        {value}
      </p>
