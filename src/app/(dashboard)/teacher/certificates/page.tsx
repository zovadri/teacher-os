      ...
      <Table
        columns={[
          { key: "studentName", header: "اسم الطالب", render: (c) => (
            <span className="text-sm font-medium text-text">{c.studentName}</span>
          )},
          ...
          { key: "actions", header: "", render: (c) => (
            <div className="flex items-center gap-1">
              <button className="p-1.5 text-text-tertiary hover:text-primary hover:bg-primary/5 rounded-lg transition-colors" title="طباعة">
                <HiOutlinePrinter size={16} />
              </button>
              <button className="p-1.5 text-text-tertiary hover:text-primary hover:bg-primary/5 rounded-lg transition-colors" title="تحميل">
                <HiOutlineDownload size={16} />
              </button>
              <button className="p-1.5 text-text-tertiary hover:text-primary hover:bg-primary/5 rounded-lg transition-colors" title="QR">
                <HiOutlineQrcode size={16} />
              </button>
              {c.status === "active" && (
                <button className="p-1.5 text-text-tertiary hover:text-error hover:bg-error/5 rounded-lg transition-colors" title="إلغاء الشهادة">
        ...
        <CardContent>
          <div className="flex items-end gap-3 h-40">
            {Array.from({ length: 6 }, (_, i) => {
              const monthHeight = 20 + Math.floor(Math.random() * 60)
              const revHeight = Math.floor(Math.random() * 15)
              return (
      ...
      <Modal isOpen={showIssueModal} onClose={() => setShowIssueModal(false)} title="إصدار شهادة جديدة" subtitle="اختر الطالب والكورس لإصدار شهادة إتمام" size="lg">
        <div className="space-y-4">
          ...
          <div className="pt-4 flex gap-3">
            <Button variant="primary" size="lg" className="flex-1">إصدار الشهادة</Button>
