...
import { HiOutlineCash, HiOutlineBanknotes, HiOutlineArrowUp, HiOutlineArrowDown, HiOutlinePlus } from "react-icons/hi"
      ...
      <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
          <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <StatsCard title="رصيد الصندوق" value="12,500 ج.م" icon={HiOutlineCash} color="success" />
            <StatsCard title="رصيد البنك" value="45,000 ج.م" icon={HiOutlineBanknotes} color="primary" />
            <StatsCard title="إجمالي المدين" value={`${totalDebit.toLocaleString()} ج.م`} icon={HiOutlineArrowUp} color="success" />
            <StatsCard title="إجمالي الدائن" value={`${totalCredit.toLocaleString()} ج.م`} icon={HiOutlineArrowDown} color="error" />
          </motion.div>
                    ...
                    {accounts.map((a) => (
                      <div key={a.name} className="flex items-center justify-between p-3 rounded-xl bg-surface-secondary border border-border">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`text-sm font-bold ${a.balance > 0 ? "text-success" : "text-error"}`}>{Math.abs(a.balance).toLocaleString()} ج.م</span>
                  ...
                  <div className="space-y-2">
                    {entries.slice(0, 6).map((e) => (
                      <div key={e.id} className="flex items-center justify-between p-3 rounded-xl bg-surface-secondary border border-border">
                        ...
                        <span className={`text-sm font-bold ${e.type === "receipt" ? "text-success" : "text-error"}`}>{e.type === "receipt" ? "+" : "-"}{e.debit || e.credit} ج.م</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
