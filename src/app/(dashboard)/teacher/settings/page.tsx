...
function PasswordInput({ label, value, onChange, id }: { label: string; value: string; onChange: (v: string) => void; id?: string }) {
  ...
  return (
    <Input
      ...
      id={id}
      leftIcon={show ? <EyeOff size={16} onClick={() => setShow(false)} className="cursor-pointer" /> : <Eye size={16} onClick={() => setShow(true)} className="cursor-pointer" />}
    />
  )
}
...
export default function SettingsPage() {
  ...
  const handleSave = async (section: string) => {
    ...
    setSaving(null)
    addToast({ type: "success", title: "تم الحفظ", message: "تم حفظ التغييرات بنجاح" })
  }

  const handleChangePassword = async () => {
    if (!passwords.current || !passwords.newPass || !passwords.confirm) {
      addToast({ type: "error", title: "خطأ", message: "يرجى ملء جميع الحقول" })
      return
    }
    if (passwords.newPass !== passwords.confirm) {
      addToast({ type: "error", title: "خطأ", message: "كلمة المرور الجديدة غير متطابقة" })
      return
    }
    setSaving("password")
    await new Promise((r) => setTimeout(r, 1200))
    ...
    addToast({ type: "success", title: "تم التغيير", message: "تم تغيير كلمة المرور بنجاح" })
  }

  return (
    <div className="p-4 md:p-6 space-y-6">
      <DashboardHeader title="الإعدادات" subtitle="إدارة إعدادات المنصة والمركز" />
                ...
                <div className="space-y-6">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>البيانات الأساسية للمركز التعليمي</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <Input
                        label="اسم المركز *"
                        value={general.centerName}
                        onChange={(e) => setGeneral((p) => ({ ...p, centerName: e.target.value }))}
                      />
                      <Textarea
                        label="وصف المركز"
                        value={general.centerDescription}
                        onChange={(e) => setGeneral((p) => ({ ...p, centerDescription: e.target.value }))}
                      />
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Select
                          ...
                          onChange={(e) => setGeneral((p) => ({ ...p, defaultLanguage: e.target.value }))}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input
                          ...
                          type="number"
                          value={String(general.trialDays)}
                          onChange={(e) => setGeneral((p) => ({ ...p, trialDays: Number(e.target.value) }))}
                        />
                        <Input
                          label="الحد الأقصى للطلاب في الكورس"
                          type="number"
                          value={String(general.maxStudentsPerCourse)}
                          onChange={(e) => setGeneral((p) => ({ ...p, maxStudentsPerCourse: Number(e.target.value) }))}
                        />
                      </div>
                    </CardContent>
                    <CardFooter>
              ...
              <TabPanel id="notifications" activeTab={activeTab}>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>تحكم في أنواع الإشعارات المرسلة</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <Toggle
                      label="إشعارات تسجيل طالب جديد"
                      enabled={notifications.newStudent}
                      onChange={(v) => setNotifications((p) => ({ ...p, newStudent: v }))}
                    />
                    <Toggle
                      label="إشعارات دفع جديد"
                      enabled={notifications.newPayment}
                      onChange={(v) => setNotifications((p) => ({ ...p, newPayment: v }))}
                    />
                    <Toggle
                      label="إشعارات نتائج الامتحانات"
                      enabled={notifications.examResults}
                      onChange={(v) => setNotifications((p) => ({ ...p, examResults: v }))}
                    />
                    <Toggle
                    ...
                    />
                    <Toggle
                      label="إشعارات انتهاء الاشتراك"
                      enabled={notifications.subscriptionExpiry}
                      onChange={(v) => setNotifications((p) => ({ ...p, subscriptionExpiry: v }))}
                    />
                    <Toggle
                      label="إشعارات الرسائل الجديدة"
                      enabled={notifications.newMessages}
                      onChange={(v) => setNotifications((p) => ({ ...p, newMessages: v }))}
                    />
                    <Toggle
                      label="تقرير أسبوعي عبر البريد الإلكتروني"
                ...
                <div className="space-y-6">
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <Toggle
                        ...
                        enabled={payment.fawryEnabled}
                        onChange={(v) => setPayment((p) => ({ ...p, fawryEnabled: v }))}
                      />
                      <Toggle
                        label="تفعيل الدفع عبر بطاقة ائتمان"
                        enabled={payment.creditCardEnabled}
                        onChange={(v) => setPayment((p) => ({ ...p, creditCardEnabled: v }))}
                      />
                      <Toggle
                        label="تفعيل المحفظة الإلكترونية"
                        enabled={payment.walletEnabled}
                        onChange={(v) => setPayment((p) => ({ ...p, walletEnabled: v }))}
                      />
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        />
                        <Select
                          label="فترة تجديد الاشتراك"
                          value={payment.renewalPeriod}
                          onChange={(e) => setPayment((p) => ({ ...p, renewalPeriod: e.target.value }))}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input
                          ...
                          type="number"
                          value={String(payment.vatPercent)}
                          onChange={(e) => setPayment((p) => ({ ...p, vatPercent: Number(e.target.value) }))}
                        />
                        <Input
                          label="نسبة الخصم القصوى المسموحة (%)"
                          type="number"
                          value={String(payment.maxDiscountPercent)}
                          onChange={(e) => setPayment((p) => ({ ...p, maxDiscountPercent: Number(e.target.value) }))}
                        />
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <Input
                        label="اسم البنك"
                        value={payment.bankAccount.bankName}
                        onChange={(e) => setPayment((p) => ({ ...p, bankAccount: { ...p.bankAccount, bankName: e.target.value } }))}
                      />
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input
                          label="رقم الحساب"
                          value={payment.bankAccount.accountNumber}
                          onChange={(e) => setPayment((p) => ({ ...p, bankAccount: { ...p.bankAccount, accountNumber: e.target.value } }))}
                        />
                        <Input
                          label="الآيبان (IBAN)"
                          value={payment.bankAccount.iban}
                          onChange={(e) => setPayment((p) => ({ ...p, bankAccount: { ...p.bankAccount, iban: e.target.value } }))}
                        />
                      </div>
                    </CardContent>
                    <CardFooter>
                ...
                <div className="space-y-6">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>تعزيز أمان الحساب بإضافة طبقة حماية إضافية</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <Toggle
                        label="تفعيل المصادقة الثنائية"
                        description="سيتم إرسال رمز تحقق إلى هاتفك عند تسجيل الدخول"
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>تحديد المدة التي تظل فيها جلسة تسجيل الدخول نشطة</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <Select
                  ...
                  <Card>
                    ...
                    <CardContent className="p-0">
                      <Table
                        columns={[
                          { key: "device", header: "الجهاز", render: (s: Record<string, unknown>) => (
                          ...
                          { key: "status", header: "الحالة", render: (s: Record<string, unknown>) => s.isCurrent ? (
                            <Badge variant="success" size="sm">الجلسة الحالية</Badge>
                          ) : (
                            <Badge variant="neutral" size="sm">نشطة</Badge>
                          )},
                        ]}
                    ...
                    <Card className="border-error/30 bg-error/5">
                      <CardContent>
                        ...
                        <div className="flex gap-3">
                          <Button variant="danger" onClick={() => { setShowEndSessionConfirm(false); addToast({ type: "success", title: "تم", message: "تم إنهاء جميع الجلسات الأخرى" }) }}>
                            تأكيد إنهاء الجلسات
                          </Button>
                          <Button variant="secondary" onClick={() => setShowEndSessionConfirm(false)}>
                            إلغاء
                          </Button>
                        </div>
                      </CardContent>
                  ...
                  <Card>
                    <CardHeader>
                      <div>
                        ...
                        <CardDescription>آخر محاولات تسجيل الدخول إلى حسابك</CardDescription>
                      </div>
                    </CardHeader>
                    <CardContent className="p-0">
                      <Table
                        columns={[
                          ...
                          { key: "status", header: "الحالة", render: (h: Record<string, unknown>) => (
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between px-4 py-3 border-t border-border">
                        <span className="text-sm text-text-tertiary">
                          الصفحة {historyPage} من {totalPages}
                        </span>
                        <div className="flex gap-1">
                          <button type="button"
                            onClick={() => setHistoryPage((p) => Math.max(1, p - 1))}
                            disabled={historyPage === 1}
                            className="p-1.5 rounded-lg hover:bg-surface-secondary disabled:opacity-30 disabled:cursor-not-allowed text-text-secondary"
                          >
                ...
                <div className="space-y-6">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>أضف روابط منصات التواصل الخاصة بالمركز</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>كيف ستظهر الروابط للطلاب وأولياء الأمور</CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        {socialPlatforms.map((platform) => {
                          ...
                          return (
                            <a
                              ...
                              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-surface-secondary border border-border hover:border-primary/30 hover:bg-primary/5 transition-all"
                            >
