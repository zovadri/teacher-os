  ...
  return (
    <>
      ...
      <section className="relative min-h-screen flex items-center bg-[#09090B] overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(91,95,246,0.06),transparent_50%)]" />
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <div className="flex flex-wrap gap-3">
                <Link href="/demo" className="px-6 py-3 bg-primary text-white font-medium rounded-xl hover:bg-[#6D70FF] transition-all text-[15px]">
                  جرب المنصة مجاناً
                </Link>
              ...
              <div className="flex items-center gap-6 mt-8">
                <div className="flex -space-x-2">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="w-8 h-8 rounded-full bg-primary/20 border-2 border-[#09090B] flex items-center justify-center text-xs font-medium text-text">
                      {i}
                    </div>
                  ))}
                </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <motion.div key={feat.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="group p-6 rounded-[24px] border border-border hover:border-primary/50 hover:scale-[1.02] transition-all duration-200">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {mockCourses.filter(c => c.status === "published").slice(0, 4).map((course, i) => (
              <motion.div key={course.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="group rounded-[24px] border border-border overflow-hidden hover:border-primary/50 hover:scale-[1.02] transition-all duration-200">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              <motion.div key={t.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="p-6 rounded-[24px] border border-border bg-surface hover:border-primary/50 transition-all duration-200">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {[
              ...
              <motion.div key={plan.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className={`relative p-8 rounded-[24px] border transition-all ${plan.popular ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}>
                ...
                <Link href="/register" className={`block w-full text-center py-3 rounded-xl font-medium transition-all text-[15px] ${plan.popular ? "bg-primary text-white hover:bg-[#6D70FF]" : "border border-border text-text-secondary hover:bg-surface-tertiary"}`}>
                  ابدأ الآن
                </Link>
              </motion.div>
            ))}
          ...
          <div className="space-y-3">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.03 }}>
                <button type="button" onClick={() => setFaqOpen(faqOpen === `faq-${i}` ? null : `faq-${i}`)} className="w-full text-right p-4 rounded-2xl border border-border bg-surface hover:border-primary/50 transition-all flex items-center justify-between gap-4">
