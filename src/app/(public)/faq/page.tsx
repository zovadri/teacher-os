...
export default function FAQPage() {
  ...
  return (
    <>
      ...
      <section className="sticky top-20 z-30 bg-surface/80 backdrop-blur-2xl border-b border-border">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-4 overflow-x-auto">
          <div className="flex items-center gap-2 whitespace-nowrap">
            ...
            {categories.map((cat) => (
              <button type="button" key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-1.5 text-xs font-medium rounded-[16px] transition-all ${activeCategory === cat ? "bg-primary text-white shadow-[0_0_12px_rgba(217,119,6,0.3)]" : "bg-card text-text-secondary border border-border hover:border-primary/20"}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div className="max-w-3xl mx-auto px-4 md:px-6 text-center">
          <motion.div {...fadeUp}>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link href="/contact" className="px-6 py-3 bg-primary text-white font-medium rounded-[16px] shadow-[0_0_20px_rgba(217,119,6,0.15)] hover:shadow-[0_0_30px_rgba(217,119,6,0.3)] hover:brightness-110 transition-all text-[15px]">
                اتصل بنا
              </Link>
